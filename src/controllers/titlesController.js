...
exports.create = (req, res) => {
    ...
    if (!alias || !id) return res.status(400).json({ error: "alias und id erforderlich." });
    if (!/^[\w-]+$/.test(alias)) return res.status(400).json({ error: "Ungültiger Alias." });
...
exports.remove = (req, res) => {
    ...
    if (!titles[alias]) return res.status(404).json({ error: "Alias nicht gefunden." });
    delete titles[alias];
    saveTitles(titles);
